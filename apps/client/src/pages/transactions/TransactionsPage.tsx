...
import { Box, Typography, IconButton, Chip, Stack, Button } from "@mui/material";
...
import { BulkUploadDialog } from "../../components/organism/BulkUploadDialog";
...
const TransactionsPage: React.FC<TransactionsPageProps> = () => {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    if (searchKeyword.trim()) {
      ...
      list = list.filter(
        (t) =>
          (t.categoryName ?? getCategoryName(t.categoryId))
            .toLowerCase()
            .includes(kw) || (t.memo ?? "").toLowerCase().includes(kw)
      );
    }
    return list;
  }, [
            ...
            >
              {transactionTypeFilter !== "all" && (
                <Chip
                  size="small"
                  label={`유형: ${
                    transactionTypeFilter === "income" ? "수입" : "지출"
                  }`}
